// Slightly modified version of multidimentional fft from Numerical Recipies in C
// which came from Nis Sarup's lecture notes.
...
void fourn(Doub* data, unsigned long* nn, unsigned long ndim, const Int isign) {
...
void rlft3(Doub* data, Doub* speq,
	const Int nn1, const Int nn2, const Int nn3, const Int isign) {
	...
	//VecDoubp spq(nn1);
	unsigned long nn[3];
	std::unique_ptr<double* []> manage_spq = std::make_unique<double* []>(nn1);
	...
	if (isign == -1) fourn(data, nn, 3, isign);
}
